...
{
    /// <summary>
    /// Vision data receiver client
    /// </summary>
    public class SSLVisionReceiver : MulticastReceiver
    {
        public SSLVisionReceiver(){}

        public SSLVisionReceiver(IPAddress address, uint port) : base(address, port)
        {
        ...
        public override object Receive()
        {
            ...
            {
                packet = ProtoBuf.Serializer.Deserialize<SSL_WrapperPacket>(ms);
            }
            return packet;
        }
        ...
        public override void Execute()
        {
            if (repReference != null)
            {
                ...
                repReference.InData.ParsePacket((SSL_WrapperPacket) received);
            }
            else
            {
